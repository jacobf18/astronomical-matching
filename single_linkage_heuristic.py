...
def assign_cluster_neg_bayes(linkage_arr: np.array, data: np.array, num_clusters):
    cluster_labels = fcluster(linkage_arr, t=num_clusters, criterion = "maxclust")
...
def find_clusters_single_linkage(cmat: np.array):
    """
    Find clusters in the catalog data.
    First, find a min spanning tree.
    Then, remove edges greedily to maximize
    the Bayes Factor.
    """
    data = np.concatenate(cmat, axis=0)
    Z = linkage(data[:,:2], method="single")

    bayes = []
    for n in range(75,data.shape[0]):
        bayes.append(assign_cluster_neg_bayes(Z, data, n))
